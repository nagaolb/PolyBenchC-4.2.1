...

{
  ...
  for (i = 0; i < w; i++)
    for (j = 0; j < h; j++) {
      if ((i * h + j) % 20 == 0) fprintf_wrapper_newline(POLYBENCH_DUMP_TARGET);
      fprintf_wrapper(POLYBENCH_DUMP_TARGET, DATA_PRINTF_MODIFIER, imgOut[i][j]);
    }
...
int main(int argc, char** argv)
{
  ...
  /* Be clean. */
  fprintf_wrapper_on_exit(0.020474);
